...
% Joint AoA and AoD for 2 Target per peak
...
for i_snr=1:nsnr_tests
    ...
    folderName = sprintf('./data/Bistatic_data_with_ToA_2Targets_1peak_AoA_AoD/Bistatic_data_with_ToA_2Targets_1peak_%d_dB/', SNRdB);
...
for i_snr=1:nsnr_tests
    SNRdB = SNRdBTest(i_snr); 
    folderName = sprintf('./data/Bistatic_data_with_ToA_2Targets_1peak_AoA_AoD_test_withCRB/');
    if ~exist(folderName, 'dir')
        [status, msg, msgID] = mkdir(folderName);
    end
    for mc=1:MC_ALT
