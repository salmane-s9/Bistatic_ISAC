% Figure 4:
% Performance of the MLP architecture trained at different SNR values
% Training the MLP at different SNRs and predicting on a range of Test SNRs
% Simulation data are saved as mat files.
...
% ========= SNR Range ===================
SNRdBTest = [5, 10, 15, 20, 25, 30, 40];
nsnr_tests = length(SNRdBTest);
MC_ALT = 10000;
q = 3; % number of targets in the scene

for i_snr=1:nsnr_tests
    SNRdB = SNRdBTest(i_snr);
    disp(['SNR: ', num2str(SNRdB), 'dB']);
    folderName = sprintf('./data/Bistatic_data_with_ToA_3targets_new/Bistatic_data_with_ToA_3targets_new_2D_%d_dB/', SNRdB);
    ...
    for mc=1:MC_ALT
        ...
        % ========== Target Parameters ====================== 
        AoA_interval = [-5,5;25,35;55,65];
        AoD_interval = [-5,5;25,35;55,65];
        dist_interval = [3, 15];
        speed_interval = [0, 25];
        for i=1:q
        ...
        distance_BS_Target = [30 45 60];
        distance_Target_radar = [1 15 45];
...
for i_snr=1:nsnr_tests
    SNRdB = SNRdBTest(i_snr);
    disp(['SNR: ', num2str(SNRdB), 'dB']);
    folderName = sprintf('./data/Bistatic_data_with_ToA_3targets_new_2D_test/');
    if ~exist(folderName, 'dir')
        [status, msg, msgID] = mkdir(folderName);
    end
